...
x=[-1.5*a:dx:1.5*a]; y=[-1.5*a:dx:1.5*a]; [XX,YY]=meshgrid(x,y); t=[0:dt:200];c=2;
P=(2/pi)^(1/2)*exp(-(XX).^2-(YY+a/3).^2).*exp(j*c*YY); T=[0:0.25:200]; n=1; alfa=[0:pi/40:2*pi];
%Define potential
V=((XX.^2+YY.^2).^2/25-XX.^2-YY.^2)/35+0.25; ga=sqrt((1/35+sqrt(1/(35^2)-4*1/25*1/35*(0.25-1)))/(2*1/25*1/35));
r=hbar*c/(2*m)*dt/dx;
%%
figure(1);
pause(1.5)
...
plot3(-y,zeros(length(x),1),(x.^4/25-x.^2)/35+0.25,'-k','LineWidth',1.3)
hold on
hold on
plot3(zeros(length(x),1),x,(x.^4/25-x.^2)/35+0.25,'-k','LineWidth',1.3)
hold on
plot3(ga*cos(alfa),ga*sin(alfa),ones(length(alfa),1),'-k','LineWidth',1.3)
