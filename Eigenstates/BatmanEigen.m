...
close(video);
%%
pcolor(XX(2:nx-1,2:ny-1),YY(2:nx-1,2:ny-1),0*real(reshape(W(:,70),nx-2,ny-2)).*J(2:nx-1,2:ny-1))
set(gca,'YDir','normal')
colormap(customMap)
shading interp
axis([-a a -0.5*a 0.5*a 0 1])
...
plot(Y*a,Z*a,'-k','LineWidth',6.2)
set(gcf,'InvertHardCopy','off','Color','black');
%text(0.85*a,-0.3*a,strcat(num2str(70)),'interpreter','latex','FontSize',40,'color','white')
clim([-max(max(W)) max(max(W))])
axis off
print(gcf,'foo1.png','-dpng','-r300');