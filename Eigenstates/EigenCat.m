...
%%
Data=Data002; ang=[0:pi/10:2*pi];
Y=smooth(Data(:,1));Z=smooth(Data(:,2));
Y=[(Y(1)+Y(end))/2;Y(2:end-1);(Y(1)+Y(end))/2];
Z=[(Z(1)+Z(end))/2;Z(2:end-1);(Z(1)+Z(end))/2];
Y=interp(Y,2); Z=interp(Z,2);
plot(Y*a,Z*a,'-b')
axis([-1.05*a 1.05*a -0.57*a 0.57*a 0 1])
f=(Y+j*Z)*a;
N=length(f);
%%
J=zeros(length(x));
...
plot(Y*a,Z*a,'-k','LineWidth',5)
set(gcf,'InvertHardCopy','off','Color','black');
clim([-max(max(W)) max(max(W))])
text(0.85*a,-0.3*a,strcat(num2str(i)),'interpreter','latex','FontSize',40,'color','white')
axis off
...
close(video);
%%
pcolor(XX(2:nx-1,2:ny-1),YY(2:nx-1,2:ny-1),real(reshape(W(:,69),nx-2,ny-2)).*J(2:nx-1,2:ny-1))
set(gca,'YDir','normal')
colormap(customMap)
shading interp
axis([-1.05*a 1.05*a -0.57*a 0.57*a 0 1])
...
set(gcf,'InvertHardCopy','off','Color','black');
text(0.85*a,-0.3*a,strcat(num2str(69)),'interpreter','latex','FontSize',40,'color','white')
clim([-max(max(W)) max(max(W))])
axis off
print(gcf,'foo3.png','-dpng','-r300');